        ...
        for (const pkg in msg.packages) {
          ...
          if (msg.packages[pkg].width && msg.packages[pkg].height && msg.packages[pkg].length && msg.packages[pkg].weight) {
            for (let q = 0; q < msg.packages[pkg].quantity; q++) {
              ...
              if (
                (typeof msg.packages[pkg].type === 'string' && msg.packages[pkg].type.toLowerCase() === 'pallet') ||
                  (typeof msg.packages[pkg].package_type === 'string' && msg.packages[pkg].package_type.toLowerCase() === 'pallet')
              ) {
                msg.packages[pkg].allowedRotation = [0, 2]
              }
              packages.push(
                {
                  item: new Item(
